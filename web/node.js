...
function addVideoPreview(nodeType) {
    chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        this.updateParameters = (params, force_update) => {
            ...
            if (!force_update &&
                !app.ui.settings.getSettingValue("Immortal.AdvancedPreviews", false)) {
                return;
            }
            if (timeout) {
        ...
        previewWidget.updateSource = function () {
            ...
            if (params.format?.split('/')[0] == 'video' ||
                ...
                if (app.ui.settings.getSettingValue("Immortal.AdvancedPreviews", false)) {
...
app.ui.settings.addSetting({
    id: "Immortal.AdvancedPreviews",
    name: "Immortal Advanced Previews",
    type: "boolean",
    defaultValue: false,
});

app.registerExtension({
    name: "Immortal.VideoUpload",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        var videolist = ["ImmortalVideoUpload", "ImAppendNode_Video", "ImVideoList"]
        var enablevideo = videolist.indexOf(nodeData?.name) >= 0
        if(nodeData?.name == "ImmortalVideoUpload" || nodeData?.name == "ImAppendNode_Video" || nodeData?.name == "ImVideoList") {
            useKVState(nodeType);
            chainCallback(nodeType.prototype, "onNodeCreated", function () {
                let new_widgets = []
                if (this.widgets) {
                    for (let w of this.widgets) {
                        ...
                        console.log(config)
                        if (w?.type == "text" && config.length > 1 && config[1].vhs_path_extensions) {
...
const _ID = "TimelineSync";
const _PREFIX = "source_";
const _TYPE = "*";
...
const _ID2 = "batchNodes";
const _PREFIX2 = "extraPrevNode";
const _TYPE2 = "NODE";
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        nodeType.prototype.onConnectionsChange = function (slotType, slot_idx, event, link_info, node_slot) {
            ...
            if (slotType === TypeSlot.Input) {
                ...
                // check that the last slot is a dynamic entry....
                let last = this.inputs[this.inputs.length - 1];
                if (last === undefined || (last.name != _PREFIX2 || last.type != _TYPE2)) {
                    this.addInput(_PREFIX2, _TYPE2);
		            // Set the unconnected slot to appear gray
                    last = this.inputs[this.inputs.length - 1];
                    if (last) {
